...
> **量纲**：带 `InTiles` 的字段单位是**格**；其余裸数字字段单位是**世界单位**
> （**1 格 = 14 世界单位**，游戏内掐表实测标定，见 `docs/unit-dimensions.md` §3 ——
> 该常数**改过一次**：原按目测取 8，现按采集车 3.51 s/格 标定为 14）。
> ⚠️ **换算只在展示层做**：产物存原始世界单位（`area.radius` / `stats.emp_radius` 等），
> 除成"格"由 `web/src/format.ts` 的 `toTiles()` 现算 —— 所以产物字段**不带 `_tiles`**
> 就是世界单位，带 `_tiles` 就是格，这个后缀就是量纲标记。
> 完整推导、逐字段换算表、外部交叉验证见 **`docs/unit-dimensions.md`**（台账 I205~I211、I224）。

| 字段 | 含义 | 单位 | 覆盖 |
| --- | --- | --- | --- |
| `visionRangeInTiles` | 视野 | 格 | 82/82 |
| `aggroRadiusInTiles` | 主动开火/索敌半径 | 格 | 78/82 |
| `avoidanceRadius` | 单位间避让半径（步兵 1.7 → 0.12 格、建筑 6 → 0.43 格） | 世界单位 | 82/82 |
| `hexReservationRadius` | 行进占位半径（2.5~5 → 0.18~0.36 格，小于一格） | 世界单位 | 60/82 |
