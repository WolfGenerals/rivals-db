 ...
 *   3. **手机上的 DPS 口径改成下拉** —— 分段按钮在窄屏占掉一整行；下拉与等级
 *      并排放得下。桌面仍用分段按钮（一眼看出是二选一，比下拉好）。同一个 `dpsMode`。
 *      口径只有「爆发 / 平均」两项 —— 游戏面板值不再作为口径（见 `state.ts` 的说明）。
 */
...
<template>
  <div class="level-controls">
    <!--
      ...
      切了**武器卡 / 表格 / 排序**的数字一起变；单位页那颗 DPS 是**游戏面板值**（固定），
      不跟着变 —— 它和这里的"实际值"是两回事，故不进这个切换。
