...
**已踩 6 次**（findings I34 / I96 附近；2026-xx 本轮又踩 2 次 —— 都是在 Lua 注释里顺手写了
`xxx` 这种反引号短语，两次都在 `_panel_lua_all2.ts`）。写 Lua 注释时描述代码用引号或书名号，
**不要用反引号**。写完**顺手 grep 一次反引号**最省事：

```powershell
Select-String -Path <脚本>.ts -Pattern '`'   # 只应看到 TS 那一侧的行与模板定界符
```
