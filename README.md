...
其他语言的重写实现放在 `core/`、`cli/`、`web/`（pnpm workspace），`reference/` 保持独立，
仅在本地或 CI 中作为校验基准运行。
...
| `data/` | —— | 两层之间的契约 |
| `core` | 无 | 纯函数，CLI 与网页共用 |
| `cli/` `web/` | core | 只做各自的 I/O |
...
由 `web/scripts/sync-data.mjs` 把 `data/` 复制到 `web/public/data/`。
